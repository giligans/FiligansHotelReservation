...
class DashboardController extends \BaseController {
	...
	*/
	public function index()
	{
		...
		$today_stats = Booking::where('created_at','>=', $yesterday)->get();
		...
		foreach($today_stats as $stats)
		{
			...
			if($stats->created_at->isToday() && $stats->status==1)
			{
				if($stats->check_in->isToday()){
					$arrival++;
				}
				if($stats->created_at->isToday())
				{
					if($stats->status==1)
					{
						$bookings++;	
	...
	//return $today_stats;
		return View::make('adminview.dashboard.index', compact('cpage','booking_recent','cancelled','bookings','pending','arrival','profit_today','departure','today_stats'));
	}
	...
	public function ajax_todayBookingSuccess()
	{
		...
		$today = date('Y-m-d');
		$today_stats = Booking::with('reservedRoom.room.roomDetails')->where('created_at','>', $today)->get();
		$cpage = 'dashboard';
		$success_bookings = [];
		foreach($today_stats as $stats)
		{	
	...
	{
		$today = date('Y-m-d');
		$today_stats = Booking::with('reservedRoom.room.roomDetails')->where('cancelled_at','>', $today)->get();
		$cpage = 'dashboard';
		$bookings = [];
	...
	{
		$today = date('Y-m-d');
		$today_stats = Booking::with('reservedRoom.room.roomDetails')->where('check_in','=', $today)->get();
		$cpage = 'dashboard';
		$bookings = [];
		foreach($today_stats as $stats)
		{	
	...
	{
		$yesterday = date('Y-m-d', strtotime('-1 days'));
		$today_stats = Booking::with('reservedRoom.room.roomDetails')->where('check_out','=', $yesterday)->get();
		$cpage = 'dashboard';
		$bookings = [];
