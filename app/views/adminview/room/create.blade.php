...
<form action="{{ URL::to('adminsite/room/create')}}" method="POST" role="form">
	<div class="form-group">
		<label for="">Type</label> <small style='color:gray'>(The name of the room. <span class='req'>Required Field</span>)</small>
		<input type="text" class="form-control" ng-model='name' name='name' id="" placeholder="Input field">
	</div>
	<div class="form-group">
		...
		<textarea class='form-control' ng-model='short_desc' name='short_desc'></textarea>
	</div>
	<div class="form-group">
		...
		<textarea class='form-control' ng-model='full_desc' name='full_desc'></textarea>
	</div>
	<div class="form-group">
		...
		<input type="text" name="max_adults" ng-model='max_adults' id="input" class="form-control" value="" length='2' required="required"  title="">
	</div>
	<div class="form-group">
		...
		<input type="text" name="max_children" ng-model='max_children' id="input" class="form-control" value="" length='2' required="required"  title="">
	</div>
	<div class="form-group">
		<label for="">Number of Rooms</label><small style='color:gray'>(The quantity of the room. <span class='required'>Required Field</span>)</small>
		<input type="text" name="no_of_rooms" ng-model='no_of_rooms' id="input" class="form-control" value="" required="required" title="">
	</div>
	<div class="form-group">
		<label for="">Beds</label><small style='color:gray'>(The number of beds per room. <span class='required'>Required Field</span>)</small>
		<input type="text" name="beds" ng-model='beds' id="input" class="form-control" value="" required="required"  title="">
	</div>
	<div class="form-group">
		...
		<input type="text" name="bathrooms" ng-model='bathrooms' id="input" class="form-control" value="" required="required" title="">
	</div>
	<div class="form-group">
		<label for="">Room Area</label><small style='color:gray'>(The room area of the room. <span class='required'>Not required</span>)</small>
		<input type="text" name="area" ng-model='area' id="input" class="form-control" value="" required="required"  title="">
	</div>
	<div class="form-group">
		<label for="">Price</label><small style='color:gray'>(The price per room. <span class='required'>Required Field</span>)</small>
		<input type="text" name="price" ng-model='price' id="input" class="form-control" value="" required="required"  title="">
	</div>
	<hr>	<label>Images: </label><small style='color:gray'>(The images of room. <span class='required'>Required Field</span>)</small><br>
	<div style='padding:10px;'>

	<div style='float:left;margin:10px;' ng-repeat='i in images' ng-show='images.length>0'>
			<img ng-src='/image/small/[[ i.photo.filename]]' alt='loading image'>
			<div style='height:10px;width;100px;text-align:center;background-color:black;height:20px;' ng-click='create_deleteImage($index)'><a href=''>delete image</a></div>
		</div>
...
<div class="clearfix">
	...
	<input type='hidden' name='images' ng-value='images' ng-model='images'>
	<button type="button" class="btn btn-warning"  ng-click='uploader.uploadAll()' ng-show='uploader.queue.length>0' ng-disabled='uploader.queue.length==0'>UPLOAD</button>
	<button type='button' ng-click='createRoom()' class="btn btn-primary" style='display:inline-block' ng-disabled='uploader.queue.length>0 || disablecreate'>Submit</button>
